    ...
    <div className="flex gap-2 border border-gray-300 rounded-md p-2 bg-white">
      <label className="px-2 py-1 text-sm border border-gray-300 rounded cursor-pointer hover:bg-gray-100">
        ...
        <input
          ...
          hidden
          onChange={(e) =>
            e.target.files && replaceImage(editor, e.target.files[0])
          }
        />
      </label>
...
export default function NewPostForm({
  ...
  const [postImages, setPostImages] = useState<{ url: string; path: string }[]>(
    [],
  );
  ...
  async function handlePublish(status: 'draft' | 'published') {
    ...
    try {
      ...
      if (!id) {
        slug = postSlug ? postSlug : await generateUniqueSlug(trimmedTitle, db);
  ...
  return (
    <div className="p-4 flex flex-col gap-6">
      ...
      {/* Optional Slug */}
      <div className="border border-gray-300 rounded-md p-4 ">
        <label className="block mb-2 font-medium">Hard-coded Slug</label>
        <input
          value={postSlug}
