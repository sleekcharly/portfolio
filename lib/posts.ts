...
export async function getPostsPage(page: number, tag?: string, cat?: string) {
  ...
  const postsRef = tag
    ...
    : cat
      ? db
          ...
          .where('deletedAt', '==', null)
      : db.collection('posts').where('deletedAt', '==', null);
  ...
  for (let i = 1; i < safePage; i++) {
    const q: FirebaseFirestore.Query<FirebaseFirestore.DocumentData> = lastDoc
      ? base.startAfter(lastDoc)
      : base.limit(POSTS_PER_PAGE);

    const snap: FirebaseFirestore.QuerySnapshot<FirebaseFirestore.DocumentData> =
      await q.get();
  ...
  const finalQ = lastDoc
    ? base.startAfter(lastDoc).limit(POSTS_PER_PAGE)
    : base.limit(POSTS_PER_PAGE);
...
export async function getRandomRelatedPosts(
  ...
  const baseQuery = await postsRef
  ...
  if (snap.size < 4) {
    ...
    snap = {
      docs: [...snap.docs, ...fallback.docs],
    } as FirebaseFirestore.QuerySnapshot;
  }

  const posts = snap.docs
    .filter((d) => d.id !== excludedPostId)
    .map((d) => {
      const data = d.data() as FirestorePost;
